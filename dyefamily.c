...
unsigned char *bluedye_crypt(unsigned char *data, unsigned char *key, unsigned char *nonce, long datalen) {
    int keylen = 32;
    int k[32] = {0};
    int s[256];
    int temp;
...
unsigned char * bluedye_random (unsigned char *buf, int num_bytes) {
    ...
    fclose(randfile);
    bluedye_crypt(buf, key, nonce, num_bytes);
}
...
unsigned char *wrzeszcz_crypt(unsigned char *data, unsigned char *key, unsigned char *nonce, long datalen, int keylen) {
    int k[256] = {0};
    int s[256];
    int temp;
    int output;
...
unsigned char * wrzeszcz_random (unsigned char *buf, int num_bytes) {
    ...
    fread(key, keylen, 1, randfile);
    fclose(randfile);
    wrzeszcz_crypt(buf, key, nonce, num_bytes, keylen);
}

unsigned char *reddye_crypt(unsigned char *data, unsigned char *key, unsigned char *nonce, long datalen, int keylen) {
    int diff = 256 - keylen;
    int k[256] = {0};
    int j = 0;
    int i = 0;
...
unsigned char * reddye_random (unsigned char *buf, int num_bytes) {
    ...
    fread(&key, keylen, 1, randfile);
    fclose(randfile);
    reddye_crypt(buf, key, nonce, num_bytes, keylen);
}
